...
public interface CareerRepository extends CrudRepository<Career, Long> {
    ...
    final String SELECT_ALL_CAREER_FALSE = "SELECT * FROM career WHERE status=false";
    ...
    @Query(SELECT_ALL_CAREER_FALSE)
    List<Career> findAllFalse();

    @Query(value="SELECT * FROM career WHERE name ILIKE '%'|| :term ||'%' AND status=true ;")
    List<Career> findByNameLikeIgnoreCase(@Param("term") String term);
